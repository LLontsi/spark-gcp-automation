#!/bin/bash
set -e

echo "=========================================="
echo "   ANALYSE TEMPÉRATURE - CI/CD WORKFLOW"
echo "=========================================="
echo ""

# Chemins
HDFS_URL="hdfs://10.0.0.10:9000"
DATA_PATH="$HDFS_URL/user/spark/temperature/data.csv"
RESULTS_PATH="$HDFS_URL/user/spark/temperature/results"
PYTHON_SCRIPT="/home/ansible/spark-jobs/analyze_temperature.py" # Chemin absolu conseillé

# Vérifier HDFS
echo "🔍 Vérification HDFS..."
if ! /opt/hadoop/current/bin/hdfs dfs -test -e /user/spark 2>/dev/null; then
    echo "❌ HDFS non accessible ou dossier /user/spark manquant!"
    exit 1
fi
echo "✅ HDFS accessible"
echo ""

# Étape 1: Générer les données si nécessaire
echo "📦 Étape 1/3: Vérification des données..."
if /opt/hadoop/current/bin/hdfs dfs -test -e "$DATA_PATH" 2>/dev/null; then
    FILE_SIZE=$(/opt/hadoop/current/bin/hdfs dfs -du -h "$DATA_PATH" | awk '{print $1, $2}')
    echo "   ✅ Données existantes ($FILE_SIZE)"
else
    echo "   ⚠️ Données manquantes."
    if [ -f "./generate_temp_2mb.sh" ]; then
        echo "   🔄 Lancement de la génération..."
        ./generate_temp_2mb.sh
    else
        echo "   ❌ Erreur: Script de génération introuvable!"
        exit 1
    fi
fi
echo ""

# Étape 2: Supprimer anciens résultats
echo "🧹 Étape 2/3: Nettoyage résultats précédents..."
/opt/hadoop/current/bin/hdfs dfs -rm -r -f "$RESULTS_PATH" 2>/dev/null || true
echo "   ✅ Nettoyage effectué"
echo ""

# Étape 3: Lancer l'analyse Spark
echo "🚀 Étape 3/3: Lancement analyse Spark..."
echo ""

START_TIME=$(date +%s)

# Utilisation de ressources modestes pour garantir que ça passe en CI/CD
/opt/spark/current/bin/spark-submit \
    --master spark://10.0.0.10:7077 \
    --deploy-mode client \
    --executor-memory 1G \
    --driver-memory 1G \
    --num-executors 2 \
    --executor-cores 1 \
    "$PYTHON_SCRIPT"

END_TIME=$(date +%s)
DURATION=$((END_TIME - START_TIME))

echo ""
echo "=========================================="
echo "           ANALYSE TERMINÉE"
echo "=========================================="
echo "⏱️  Durée totale: ${DURATION}s"
echo ""

# Vérification et Affichage des résultats
echo "📄 RÉSULTAT TEXTE :"
# Note: Spark crée un dossier result_txt contenant part-00000
/opt/hadoop/current/bin/hdfs dfs -cat "$RESULTS_PATH/result_txt/part-00000" 2>/dev/null || echo "❌ Pas de résultat texte trouvé"

echo ""
echo "📄 RÉSULTAT JSON :"
/opt/hadoop/current/bin/hdfs dfs -cat "$RESULTS_PATH/result_json/part-00000" 2>/dev/null || echo "❌ Pas de résultat JSON trouvé"

echo ""
echo "✅ CI/CD Success!"